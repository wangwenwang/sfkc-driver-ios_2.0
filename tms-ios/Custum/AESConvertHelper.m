...
+ (NSString *)convertToAesWithParam:(id)param {
    ...
    if ([param isKindOfClass:[NSDictionary class]]) {
        ...
        param = [NSString stringWithFormat:@"{entity:%@}",param];
        paramString = param;
    }
...
//       return nil;
//    }
    NSLog(@"解密后参数:%@",[[NSString alloc]initWithData:encryptedData encoding:NSUTF8StringEncoding]);
    NSDictionary *paramDic = [NSJSONSerialization JSONObjectWithData:encryptedData options:0 error:nil];
    if([[paramDic objectForKey:@"code"] integerValue] == 0) {
        paramDic = paramDic[@"entity"];
    }
    return paramDic;
}
